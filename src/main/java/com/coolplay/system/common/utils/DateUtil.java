...
public class DateUtil {
    ...
    public static int DateToInt(Date date) {
        return (int) (DateToLong(date) / 1000);
    }
    ...
    public static void main(String args[]) {
        ...
        Date date = StringToDate("2019-06-01");
        String hql = "(select distinct user_id from dvd_data_analysis.d_bi_user_active_record where first_sales >= {0} and first_sales < {1}) d_bi_user_active_record";

        hql = MessageFormat.format(hql, String.valueOf(date.getTime() / 1000), date.getTime() / 1000);

        System.out.println(hql);

        Calendar calendar = Calendar.getInstance();
        System.err.println("年：:" + calendar.get(Calendar.YEAR));//年
        System.err.println("月：" + (calendar.get(Calendar.MONTH) + 1));//月，加一，默认一月为0
        System.err.println("日：" + calendar.get(Calendar.DAY_OF_MONTH));//日期
        System.err.println("12小时：" + calendar.get(Calendar.HOUR));//12小时制
        System.err.println("24小时：" + calendar.get(Calendar.HOUR_OF_DAY));//24小时制
        System.err.println("分：" + calendar.get(Calendar.MINUTE));//分
        System.err.println("秒：" + calendar.get(Calendar.SECOND));//秒
        System.err.println("毫秒：" + calendar.get(Calendar.MILLISECOND));//毫秒

    }

    public static Date getWeekFirstDay(Date date) {
        Calendar cal = Calendar.getInstance();
        if(getWeek(date).getNumber()==7){
            date = addDay(date, -1);
        }
        cal.setTime(date);
    ...
    public static Date getWeekFirstDay(String date) {
        Calendar cal = Calendar.getInstance();
        if(getWeek(date).getNumber()==7){
            date = addDay(date, -1);
        }
